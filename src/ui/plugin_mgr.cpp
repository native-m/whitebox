...
static uint32_t get_plugin_flag_type(uint32_t flag) {
  switch (flag) {
    case (PluginFlags::Analyzer): return 0;
    case (PluginFlags::Effect): return 1;
    case (PluginFlags::Instrument): return 2;
    case (PluginFlags::Instrument | PluginFlags::Effect): return 3;
    default: break;
  }
  return 4;
}
...
void GuiPluginManager::render() {
  ...
  if (auto ret = confirm_dialog(
          "Delete##delete_plugin_conf", "Are you sure you want to delete the selected plugins?", ConfirmDialog::YesNo)) {
  ...
  if (ImGui::BeginTable("plugin_table", 7, table_flags)) {
    ...
    ImGui::TableSetupColumn(
        "Name", ImGuiTableColumnFlags_DefaultSort | ImGuiTableColumnFlags_WidthFixed, 0.0f, PluginManagerColumnID_Name);
    ...
    ImGui::TableSetupColumn(
        "Version", ImGuiTableColumnFlags_NoSort | ImGuiTableColumnFlags_WidthFixed, 0.0f, PluginManagerColumnID_Version);
    ImGui::TableSetupColumn(
        "Hidden", ImGuiTableColumnFlags_NoSort | ImGuiTableColumnFlags_WidthFixed, 0.0f, PluginManagerColumnID_Hidden);
    ...
    for (uint32_t id = 0; auto& plugin_info : plugin_infos) {
      ...
      switch (plugin_info.flags & 7) {
        case (PluginFlags::Analyzer): ImGui::TextUnformatted("Anayzer"); break;
        case (PluginFlags::Effect): ImGui::TextUnformatted("Effect"); break;
        case (PluginFlags::Instrument): ImGui::TextUnformatted("Instrument"); break;
        case (PluginFlags::Instrument | PluginFlags::Effect): ImGui::TextUnformatted("Instrument/Effect"); break;
      ...
      ImGui::TableNextColumn();
      ImGui::TextUnformatted(plugin_info.version.c_str(), plugin_info.version.c_str() + plugin_info.version.size());
  ...
  auto sort_predicate = [sort_specs](const PluginInfo& a, const PluginInfo& b) {
    ...
    switch (sort_spec->ColumnUserID) {
      ...
      case PluginManagerColumnID_Vendor:
        return std::lexicographical_compare(a.vendor.begin(), a.vendor.end(), b.vendor.begin(), b.vendor.end(), ch_pred);
      ...
      case PluginManagerColumnID_Path:
        return std::lexicographical_compare(a.path.begin(), a.path.end(), b.path.begin(), b.path.end(), ch_pred);
      default: WB_UNREACHABLE();
    }
    return true;
  };
