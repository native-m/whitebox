...
void TimelineBase::render_horizontal_scrollbar() {
  ...
  ImGui::SetCursorPosX(math::max(vsplitter_size, vsplitter_min_size) + 2.0f);
    ...
    bool show_selection_range)
{
  ...
  for (int i = 0; i <= line_count; i++) {
    ...
    dl->AddLine(ImVec2(rounded_gridline_pos_x, tick_pos_y - 8.0f), ImVec2(rounded_gridline_pos_x, tick_pos_y - 3.0f), col);
