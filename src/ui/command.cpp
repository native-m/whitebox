...
void MidiCmd::undo(uint32_t channel) {
  Track* track = g_engine.tracks[track_id];
  Clip* clip = track->clips[clip_id];
  ...
  midi_data->create_metadata(restored_notes, deleted_notes.size());
  midi_data->update_channel(channel);
}
...
bool MidiResizeNoteCmd::execute() {
  if (selection) {
    backup(g_engine.resize_selected_note(track_id, clip_id, relative_pos, left_side));
  } else {
    backup(g_engine.resize_note(track_id, clip_id, note_id, relative_pos, left_side));
  }
  return true;
}

void MidiResizeNoteCmd::undo() {
  std::unique_lock lock(g_engine.editor_lock);
  MidiCmd::undo(0);
}
...
bool MidiChangeNoteVelocityCmd::execute() {
  Track* track = g_engine.tracks[track_id];
  Clip* clip = track->clips[clip_id];
  MidiData* data = clip->get_midi_data();
  MidiNoteBuffer& note_seq = data->note_sequence;
  std::unique_lock lock(g_engine.editor_lock);
  old_velocity = note_seq[note_id].velocity;
  note_seq[note_id].velocity += relative_velocity;
  return true;
}

void MidiChangeNoteVelocityCmd::undo() {
  Track* track = g_engine.tracks[track_id];
  Clip* clip = track->clips[clip_id];
  MidiData* data = clip->get_midi_data();
  MidiNoteBuffer& note_seq = data->note_sequence;
