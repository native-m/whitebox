...
void GuiPianoRoll::render() {
  ...
  if (ImGui::BeginChild("PianoRoll", ImVec2(), ImGuiChildFlags_AlwaysUseWindowPadding)) {
    ...
    draw_list->AddLine(
        ...
        ImVec2(content_origin.x + child_content_size.x, content_origin.y - 1.0f),
    ...
    ImGui::InvisibleButton("PianoRollKeys", ImVec2(vsplitter_min_size, note_count * note_height_padded));
    ...
    for (int i = oct_count; i >= 0; i--) {
      draw_piano_keys(draw_list, oct_pos, ImVec2(vsplitter_min_size, note_height), i + key_oct_offset);
    }
...
void GuiPianoRoll::render_editor() {
  ...
  for (uint32_t i = 0; i <= guidestrip_count; i++) {
    ...
    if (i % 2) {
      draw_list->AddRectFilled(
          ImVec2(start_pos_x, offset_y), ImVec2(guidestrip_pos_x, offset_y + region_size.y), guidestrip_color);
    }
  }
  ...
  for (int i = 0; i <= gridline_count; i++) {
    ...
    draw_list->AddLine(
        ImVec2(gridline_pos_x_pixel, offset_y), ImVec2(gridline_pos_x_pixel, offset_y + region_size.y), line_color, 1.0f);
  }
  ...
  for (int i = 0; i <= num_keys; i++) {
    ...
    if (note_semitone % 2 == 0) {
      draw_list->AddRectFilled(
          key_pos + ImVec2(0.0f, 1.0f), key_pos + ImVec2(timeline_width, note_height_padded), guidestrip_color);
    }
  ...
  for (auto& note : midi_note.channels[0]) {
    ...
    draw_list->AddText(
        font,
        font->FontSize,
        ImVec2(std::max(cursor_pos.x, min_pos_x) + 3.0f, a.y + 2.0f),
        text_color,
        note_name,
        nullptr,
        0.0f,
