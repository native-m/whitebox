...
bool timeline_scrollbar(
    ...
    double* start_pos,
    double* end_pos) {
  assert(start_pos != nullptr && end_pos != nullptr);
  ...
  if (ImGui::Button("<", btn_size)) {
    double new_scroll_pos = math::max(*start_pos - step_size, 0.0);
    *end_pos = new_scroll_pos + (*end_pos - *start_pos);
    *start_pos = new_scroll_pos;
  }
  ...
  double start_pos_val = *start_pos;
  double end_pos_val = *end_pos;
  ...
  switch (state) {
    case TimelineScrollbarInstance::ScrollLeftSide: {
      ...
      ImGui::SetMouseCursor(ImGuiMouseCursor_ResizeEW);
      *start_pos = math::clamp(old_scroll_pos + drag_delta.x / scroll_bar_max_length, 0.0, end_pos_val - min_space);
      break;
    }
    case TimelineScrollbarInstance::ScrollRightSide: {
      ...
      ImVec2 drag_delta = ImGui::GetMouseDragDelta();
      ImGui::SetMouseCursor(ImGuiMouseCursor_ResizeEW);
      *end_pos = math::max(old_scroll_pos + drag_delta.x / scroll_bar_max_length, start_pos_val + min_space);
      break;
    }
...
timeline_ruler(const char* str_id, float width, double song_length, double* start_pos, double* end_pos, double* time_pos) {
  ...
  double start_pos_val = *start_pos;
  double end_pos_val = *end_pos;
  double view_scale = timeline_calc_view_scale(start_pos_val, end_pos_val, song_length, width);
  ...
  if (lmb_clicked) {
    double target_pos = (double)(mouse_pos.x - cursor_pos.x) / song_length * view_scale + start_pos_val;
  ...
  auto zoom = [&](float mouse_pos_x, float direction, float rate) {
    ...
    auto [new_start_pos, new_end_pos] =
        timeline_zoom(direction * rate, zoom_position, song_length, view_scale, start_pos_val, end_pos_val);
    view_scale = timeline_calc_view_scale(new_start_pos, new_end_pos, song_length, width);
    *start_pos = start_pos_val = new_start_pos;
    *end_pos = end_pos_val = new_end_pos;
  };
  ...
  if (mmb_released) {
    ...
    view_scale = timeline_calc_view_scale(start_pos_val, end_pos_val, song_length, width);
    ret = TimelineRulerResult::Zoom;
  }
  ...
  float scroll_pos_x = (float)std::round(start_pos_val * song_length * inv_view_scale);
