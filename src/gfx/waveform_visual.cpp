        ...
        switch (quality) {
            case WaveformVisualQuality::Low:
                for (uint32_t i = 0; i < sample->channels; i++) {
                    ...
                    summarize_for_mipmaps_impl(sample->format, sample->count, data, chunk_count, block_count,
                                               mip_data_count, (int8_t*)upload_ptr + offset);
                }
                break;
            case WaveformVisualQuality::High:
                for (uint32_t i = 0; i < sample->channels; i++) {
                    ...
                    std::byte* data = sample->sample_data[i];
                    summarize_for_mipmaps_impl(sample->format, sample->count, data, chunk_count, block_count,
                                               mip_data_count, (int16_t*)upload_ptr + offset);
                }
                break;
        }
...
void gfx_draw_waveform_batch(const Vector<WaveformDrawCmd>& commands, int32_t clip_x0, int32_t clip_y0, int32_t clip_x1,
                             int32_t clip_y1) {
    if (commands.size() == 0)
        return;
