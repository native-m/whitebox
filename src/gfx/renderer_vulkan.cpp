...
static VkFormat get_vk_format(GPUFormat format) {
  switch (format) {
    case GPUFormat::UnormR8G8B8A8: return VK_FORMAT_R8G8B8A8_UNORM;
    case GPUFormat::UnormB8G8R8A8: return VK_FORMAT_B8G8R8A8_UNORM;
    case GPUFormat::FloatR32G32: return VK_FORMAT_R32G32_SFLOAT;
    case GPUFormat::FloatR32G32B32: return VK_FORMAT_R32G32B32_SFLOAT;
...
static VkRenderPass create_render_pass_for_format(VkDevice device, GPUFormat format) {
  ...
  VkSubpassDependency subpass_dependency{
    ...
    .srcAccessMask = VK_ACCESS_SHADER_WRITE_BIT | VK_ACCESS_COLOR_ATTACHMENT_READ_BIT | VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT,
...
VkDescriptorSet GPUDescriptorStreamVK::allocate_descriptor_set(
    VkDevice device,
...
GPUDescriptorStreamChunkVK*
GPUDescriptorStreamVK::create_chunk(VkDevice device, uint32_t max_descriptor_sets, uint32_t max_descriptors) {
...
bool GPURendererVK::init(SDL_Window* window) {
  ...
  VK_CHECK(
      vmaFindMemoryTypeIndexForBufferInfo(allocator_, &staging_buffer_info, &staging_buffer_alloc_info, &memory_type_index));
...
GPUBuffer* GPURendererVK::create_buffer(
  ...
  if (contain_bit(usage, GPUBufferUsage::Writeable)) {
    ...
    for (uint32_t i = 0; i < num_inflight_frames_; i++) {
      VkResult result = vmaCreateBuffer(
          allocator_, &buffer_info, &allocation_info, &new_buffer->buffer[i], &new_buffer->allocation[i], &alloc_result);
      if (VK_FAILED(result)) {
...
GPUTexture* GPURendererVK::create_texture(
  ...
  if (contain_bit(usage, GPUTextureUsage::RenderTarget)) {
    ...
    for (uint32_t i = 0; i < num_inflight_frames_; i++) {
      VkResult result = vmaCreateImage(
          allocator_, &image_info, &allocation_info, &new_texture->image[i], &new_texture->allocation[i], nullptr);
      if (VK_FAILED(result)) {
        for (uint32_t j = 0; j < i; j++)
...
void GPURendererVK::set_shader_parameter(size_t size, const void* data) {
  ...
  vkCmdPushConstants(
      current_cb_, pipeline->layout, VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0, (uint32_t)size, data);
}
  ...
  if (dirty_flags.texture != 0 || dirty_flags.storage_buf != 0) {
    ...
    if (current_pipeline_layout_ != VK_NULL_HANDLE) {
      ...
      if (current_descriptor_sets_[1]) {
        vkCmdBindDescriptorSets(
            cb, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline->layout, 1, 1, &current_descriptor_sets_[1], 0, nullptr);
      }
    }
...
void GPURendererVK::enqueue_resource_upload_(
    VkImage image,
    ...
    switch (item.type) {
      ...
      case GPUUploadItemVK::Image: {
        ...
        vkCmdPipelineBarrier(
            upload_cb, old_access.stages, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr, 0, nullptr, 1, &barrier);

        VkBufferImageCopy region{
          .imageSubresource = { .aspectMask = VK_IMAGE_ASPECT_COLOR_BIT, .layerCount = 1 },
          .imageExtent = { (uint32_t)item.width, item.height, 1 },
        };
        vkCmdCopyBufferToImage(upload_cb, item.src_buffer, item.dst_image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);
        ...
        vkCmdPipelineBarrier(
            upload_cb, VK_PIPELINE_STAGE_TRANSFER_BIT, new_access.stages, 0, 0, nullptr, 0, nullptr, 1, &barrier);
        break;
      }
    }
...
void GPURendererVK::bind_resources_(VkCommandBuffer cmd_buf) {
  ...
  if (uint32_t dirty_bits = dirty_flags.texture) {
    ...
    if (num_barriers > 0) {
      ...
      if (render_pass_started_)
        end_render_pass_();
      vkCmdPipelineBarrier(
          cmd_buf, src_stage, dst_stage, VK_DEPENDENCY_BY_REGION_BIT, 0, nullptr, 0, nullptr, num_barriers, barriers);
    }
  ...
  if (uint32_t dirty_bits = dirty_flags.storage_buf) {
    uint32_t num_descriptors = 0;
    VkDescriptorSet descriptor_set = descriptor_stream_.allocate_descriptor_set(device_, storage_buffer_set_layout_, 4, 0);
  ...
  if (num_descriptor_set_updates > 0) {
    ...
    vkCmdBindDescriptorSets(
        ...
        descriptor_set_first_slot,
        num_descriptor_set_updates,
        &current_descriptor_sets_[descriptor_set_first_slot],
        0,
        nullptr);
    current_pipeline_layout_ = pipeline->layout;
  }
...
void GPURendererVK::dispose_resources_(uint64_t frame_count) {
  ...
  while (!resource_disposal_.empty()) {
    ...
    if (item.frame_stamp + num_inflight_frames_ < frame_count) {
      switch (item.type) {
        case GPUResourceDisposeItemVK::Buffer: vmaDestroyBuffer(allocator_, item.buffer.buffer, item.buffer.allocation);
...
#endif
          break;
        case GPUResourceDisposeItemVK::Pipeline: vkDestroyPipeline(device_, item.pipeline.pipeline, nullptr);
#if WB_LOG_VULKAN_RESOURCE_DISPOSAL
...
#endif
          break;
        case GPUResourceDisposeItemVK::SyncObject: vkDestroySemaphore(device_, item.sync_obj.semaphore, nullptr);
#if WB_LOG_VULKAN_RESOURCE_DISPOSAL
