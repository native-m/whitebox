...
struct GPURenderer {
  using DrawFn =
      void (*)(void* private_data, uint32_t vtx_count, uint32_t instance_count, uint32_t first_vtx, uint32_t first_instance);

  using DrawIndexedFn = void (*)(
      void* private_data,
      uint32_t idx_count,
  ...
  virtual GPUBuffer* create_buffer(
      GPUBufferUsageFlags usage,
      size_t buffer_size,
  ...
  virtual GPUTexture* create_texture(
      GPUTextureUsageFlags usage,
      GPUFormat format,
      uint32_t w,
      uint32_t h,
