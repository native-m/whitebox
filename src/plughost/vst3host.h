...
class VST3InputEventList : public Steinberg::Vst::IEventList {
 public:
  ...
  Steinberg::int32 PLUGIN_API getEventCount() SMTG_OVERRIDE {
    return event_list ? event_list->size() : 0;
  }
...
struct VST3PluginWrapper : public PluginInterface, public Steinberg::Vst::IComponentHandler, public Steinberg::IPlugFrame {
  std::string name_;
  ...
  PluginResult init_processing(PluginProcessingMode mode, uint32_t max_samples_per_block, double sample_rate) override;
  ...
  PluginResult render_ui() override {
