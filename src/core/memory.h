...
namespace wb
{
    ...
    inline void* allocate_aligned(size_t size, size_t alignment) noexcept
    {
    ...
    struct Pool
    {
        ...
        static constexpr size_t aligned_size = alloc_size + (alloc_alignment - alloc_size % alloc_alignment) % alloc_alignment;
        ...
        bool _reserve_new_block() noexcept
        {
            ...
            for (std::size_t i = 1; i < objects_per_block; i++) {
                current_chunk->next_chunk = reinterpret_cast<PoolChunk*>(reinterpret_cast<uint8_t*>(current_chunk) + aligned_size);
