...
void convert_f32_to_interleaved_i16(
    int16_t* dst,
...
void convert_f32_to_interleaved_i24(
    std::byte* dst,
    const float* const* src,
    size_t src_offset,
    size_t num_samples,
...
void convert_f32_to_interleaved_i24_x8(
    int32_t* dst,
    const float* const* src,
    size_t src_offset,
    size_t num_samples,
...
void convert_f32_to_interleaved_i32(
    int32_t* dst,
    const float* const* src,
    size_t src_offset,
    size_t num_samples,
  ...
  for (uint32_t c = 0; c < num_channels; c++) {
    ...
    for (size_t i = 0; i < num_samples; i++) {
      float src_sample = src_channel[i];
      dst[i * num_channels + c] = (int32_t)(src_sample > 0.0f ? (double)src_sample * max_val : (double)src_sample * min_val);
    }
  }
}

void convert_to_interleaved_f32(
    float* dst,
    const float* const* src,
    size_t src_offset,
    size_t num_samples,
...
void convert_to_deinterleaved_f32(
    float* const* dst,
    const float* src,
    size_t dst_offset,
    size_t num_samples,
