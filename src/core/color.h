...
inline static constexpr ImColor color_brighten(const ImColor& color, float amount) {
    amount = 1.0f / (1.0f + amount);
    return ImColor(1.0f - (1.0f - color.Value.x) * amount, 1.0f - (1.0f - color.Value.y) * amount,
                   1.0f - (1.0f - color.Value.z) * amount, color.Value.w);
}

inline static constexpr ImColor color_adjust_contrast(const ImColor& color, float amount) {
    return ImColor((color.Value.x - 0.5f) * amount + 0.5f, (color.Value.y - 0.5f) * amount + 0.5f,
                   (color.Value.z - 0.5f) * amount + 0.5f, color.Value.w);
}
...
inline static constexpr ImColor color_premul_alpha(const ImColor& color) {
    return ImColor(color.Value.x * color.Value.w, color.Value.y * color.Value.w,
                   color.Value.z * color.Value.w, 1.0f);
}
