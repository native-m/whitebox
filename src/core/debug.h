...
namespace wb
{
    ...
    public:
        ...
        {
            auto file_sink =
                std::make_shared<spdlog::sinks::rotating_file_sink_mt>("logs/imdev-log.txt",
                                                                       1048576, 3, false,
    ...
    static void report_check(const char* expr_str, const char* file, const char* func, uint32_t line_number)
    {
...
#define WB_CHECK(x) \
    (void)((!!(x)) || (report_check(#x, __FILE__, __func__, __LINE__), 0))
