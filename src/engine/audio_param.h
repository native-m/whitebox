...
struct AudioParameterList {
  ...
  inline void resize(uint32_t param_count) {
    params.resize(param_count);
  }

  inline void update() {
    params_updated.store(true, std::memory_order_release);
  }
  ...
  inline float get_plain_float(uint32_t id) const {
    return params[id].f32_norm.plain;
  }
